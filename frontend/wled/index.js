...
function loadBg(iUrl) {
	...
	if (iUrl == "") {
		var today = new Date();
		if (today.getMonth() == 11 && today.getDate() > 23 && today.getDate() < 28)
			img.src = "https://aircoookie.github.io/xmas.png";
	}
...
function presetError(empty) {
	...
	if (empty) cn += `You have no presets yet!`;
	else cn += `Sorry, there was an issue loading your presets!`;
	...
	if (hasBackup) {
		...
		else cn += `Here is a backup of the last known good state:`;
...
function populatePresets(fromls) {
	...
	for (var key of arr || []) {
		...
		cn += `<div class="segname pname" onclick="setPreset(${i})">${pName(i)}</div>
			<i class="icons e-icon flr ${expanded[i + 100] ? "exp" : ""}" id="sege${i + 100}" onclick="expand(${
			i + 100
		})">&#xe395;</i>
...
function populateInfo(i) {
  ...
  ${inforow("Frames / second", i.leds.fps)}
	...
	${inforow(
		"Filesystem",
		i.fs.u + "/" + i.fs.t + " kB (" + Math.round((i.fs.u * 100) / i.fs.t) + "%)"
	)}
...
function populateSegments(s) {
	...
	for (var y = 0; y < (s.seg || []).length; y++) {
		...
		cn += `<div class="seg">
			...
			<i class="icons e-icon flr ${
				expanded[i] ? "exp" : ""
			}" id="sege${i}" onclick="expand(${i})">&#xe395;</i>
			...
			<table class="segt">
				...
				<tr>
					<td class="segtd"><input class="noslide segn" id="seg${i}s" type="number" min="0" max="${
			ledCount - 1
		}" value="${inst.start}" oninput="updateLen(${i})"></td>
					<td class="segtd"><input class="noslide segn" id="seg${i}e" type="number" min="0" max="${ledCount}" value="${
			inst.stop
		}" oninput="updateLen(${i})"></td>
				</tr>
			</table>
			<table class="segt">
				<tr>
				...
				<tr>
					<td class="segtd"><input class="noslide segn" id="seg${i}grp" type="number" min="1" max="255" value="${
			inst.grp
		}" oninput="updateLen(${i})"></td>
					<td class="segtd"><input class="noslide segn" id="seg${i}spc" type="number" min="0" max="255" value="${
			inst.spc
		}" oninput="updateLen(${i})"></td>
				</tr>
			</table>
			<div class="h bp" id="seg${i}len"></div>
			<i class="icons e-icon pwr ${
				powered[i] ? "act" : ""
			}" id="seg${i}pwr" onclick="setSegPwr(${i})">&#xe08f;</i>
			<div class="sliderwrap il sws">
				<input id="seg${i}bri" class="noslide sis" onchange="setSegBri(${i})" oninput="updateTrail(this)" max="255" min="1" type="range" value="${
			inst.bri
		}" />
	...
	if (lowestUnused >= maxSeg) {
		d.getElementById("segutil").innerHTML =
...
function populateNodes(i, n) {
	...
	if (n.nodes) {
		...
		for (var x = 0; x < n.nodes.length; x++) {
			var o = n.nodes[x];
			if (o.name) {
				var url = `<button class="btn btna-icon tab" onclick="location.assign('http://${
					o.ip
				}');">${bname(o)}</button>`;
...
function updatePA() {
	...
	if (currentPreset > 0) {
		...
		if (acv && !expanded[currentPreset + 100]) acv.style.background = "var(--c-6)";
...
function displayRover(i, s) {
	d.getElementById("rover").style.transform =
		i.live && s.lor == 0 ? "translateY(0px)" : "translateY(100%)";
	...
	fetch(url, {
		...
		.then((json) => {
			...
			if (!command || rinfo) {
				...
				pmtLast = pmt;
				d.getElementById("buttonNodes").style.display =
					info.ndc > 0 && window.innerWidth > 770 ? "block" : "none";
				lastinfo = info;
				if (isInfo) {
					populateInfo(info);
				}
			...
			var cd = d.getElementById("csl").children;
			for (let e = 2; e >= 0; e--) {
				cd[e].style.backgroundColor =
					"rgb(" + i.col[e][0] + "," + i.col[e][1] + "," + i.col[e][2] + ")";
...
function toggleNl() {
	...
	if (nlA) {
		showToast(
			`Timer active. Your light will turn ${nlTar > 0 ? "on" : "off"} ${
				nlFade ? "over" : "after"
			} ${nlDur} minutes.`
		);
	} else {
	...
	var cn = `<div class="seg">
			...
			<div class="segin expanded">
				<table class="segt">
					...
					<tr>
						<td class="segtd"><input class="noslide segn" id="seg${lowestUnused}s" type="number" min="0" max="${
		ledCount - 1
	}" value="${ns}" oninput="updateLen(${lowestUnused})"></td>
...
function makeP(i) {
	return `
	<input type="text" class="ptxt noslide" id="p${i}txt" autocomplete="off" maxlength=32 value="${
		i > 0 ? pName(i) : ""
	}" placeholder="Enter name..."/><br>
	<div class="c">Quick load label: <input type="text" class="stxt noslide" maxlength=2 value="${qlName(
		i
	)}" id="p${i}ql" autocomplete="off"/></div>
	...
	<div class="c">Save to ID <input class="noslide" id="p${i}id" type="number" oninput="checkUsed(${i})" max=250 min=1 value=${
		i > 0 ? i : getLowestUnusedP()
	}></div>
	<div class="c">
		<button class="btn btn-i btn-p" onclick="saveP(${i})"><i class="icons btn-icon">&#xe390;</i>${
		i > 0 ? "Save changes" : "Save preset"
	}</button>
		${
			i > 0
				? '<button class="btn btn-i btn-p" onclick="delP(' +
				  i +
				  ')"><i class="icons btn-icon">&#xe037;</i>Delete preset</button>'
...
function toggleCY() {
	...
	if (d.getElementById("cyToggle").checked) {
		obj = {
			pl: 0,
			ccnf: {
				min: parseInt(d.getElementById("cycs").value),
				max: parseInt(d.getElementById("cyce").value),
				time: parseInt(d.getElementById("cyct").value * 10),
			},
		};
		obj.transition = parseInt(d.getElementById("cyctt").value * 10);
	}
...
function setColor(sr) {
	var cd = d.getElementById("csl").children;
	if (sr == 1 && cd[csel].style.backgroundColor == "rgb(0, 0, 0)")
		cpick.color.setChannel("hsv", "v", 100);
...
function search(searchField) {
	...
	searchField.parentElement.getElementsByClassName("search-cancel-icon")[0].style.display =
		searchText.length < 1 ? "none" : "inline";
...
function expand(i, a) {
	...
	if (i > 100) {
		//presets
		var p = i - 100;
		d.getElementById(`p${p}o`).style.background =
			expanded[i] || p != currentPreset ? "var(--c-2)" : "var(--c-6)";
